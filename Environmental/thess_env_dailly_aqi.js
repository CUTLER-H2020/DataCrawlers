        ...
        xlData.forEach(res => {
          ...
          for (var key in res) {
            if (
              ...
              .indexOf('Σταθμός') < 0 &&
              key.replace(' -', '').replace('\r\n', '') != 'Ημερομηνία' &&
              !key.includes('Αιθάλη') && !key.includes('H2S')
            ) {
              ...
              if (greekUtils.toGreeklish(val.split('Στ. ')[1]).split(' ').length > 1 &&
                locations[greekUtils.toGreeklish(val.split('Στ. ')[1]).split(' ')[1]]
              ) {
                coords = locations[greekUtils.toGreeklish(val.split('Στ. ')[1]).split(' ')[1]];
              } else if (
                locations[greekUtils.toGreeklish(val.split('Στ. ')[1])]
              ) {
                coords = locations[greekUtils.toGreeklish(val.split('Στ. ')[1])];
              }

              var keyVal = parseInt(res[key]) % 1 === 0 ? res[key] : parseInt(res[key]);
              ...
              var returnVal = {
                ...
                parameter_fullname: splited.length > 2 ? name.replace(' - ', '') + " " + splited[2] : name.replace(' - ', '') + " " + splited[1],
              ...
              // if (name.replace(' - ', '') == 'PM10' || name.replace(' - ', '') == 'PM2,5') {
                ...
                let selectedBp = breakpoints.filter(bp => {
                  return pm10value <= bp['PM10'].high && pm10value >= bp['PM10'].low
                })[0]

                let selectedBp25;

                if (selectedBp) {
                  pm10AQI = ((selectedBp['AQI'].high - selectedBp['AQI'].low) / (selectedBp['PM10'].high - selectedBp['PM10'].low)) * (pm10value - selectedBp['PM10'].low) + selectedBp['AQI'].low;
                }

                if (pm25value) {
                  selectedBp25 = breakpoints.filter(bp => {
                    return pm25value <= bp['PM25'].high && pm25value >= bp['PM25'].low
                  })[0]

                  pm25AQI = ((selectedBp25['AQI'].high - selectedBp25['AQI'].low) / (selectedBp25['PM25'].high - selectedBp25['PM25'].low)) * (pm25value - selectedBp25['PM25'].low) + selectedBp25['AQI'].low;
